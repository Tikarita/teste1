# train_yolo_dataset_14classes.py
from ultralytics import YOLO
import os
from pathlib import Path

# Configurações
DATASET_DIR = "dados"  # ← MUDADO DE panoramic_radiography_yolo_dataset_14_classes PARA dados
DATA_YAML = f"{DATASET_DIR}/data.yaml"
MODEL_NAME = "yolov8m.pt"
EPOCHS = 100
BATCH = 8
IMGSZ = 640
WORKERS = 0  # ⚠️ 0 para Windows, 4+ para Linux
...
def train_yolo():
    """Treina YOLOv8 para o dataset de 14 classes."""
    
    print("\n" + "="*60)
    print("🚀 INICIANDO TREINAMENTO YOLOv8 - 14 CLASSES")
    print("="*60 + "\n")
    ...
    print(f"   Device: cuda (se disponível)")
    ...
    try:
        results = model.train(
            ...
            batch=BATCH,
            imgsz=IMGSZ,
            workers=WORKERS,  # ⚠️ CRÍTICO PARA WINDOWS
            device=0,  # GPU 0
            verbose=True,
            exist_ok=True,
            project="runs_qc",
            name="yolov8_panorama_14classes",
            pretrained=True,
            optimizer="AdamW",
            lr0=0.001,
            patience=30,
        ...
        print("="*60)
        print(f"\n📁 Modelo salvo em: runs_qc/yolov8_panorama_14classes/weights/best.pt")
